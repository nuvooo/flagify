...
export default function ProjectSettings() {
  ...
  const handleUpdateEnvironment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setEnvironments(
        environments.map((e) =>
          e.id === editingEnv.id ? response.data.environment : e
        )
      )
      setShowEnvDialog(false)
      setEditingEnv(null)
  ...
  const handleDeleteEnvironment = async (envId: string) => {
    if (
      !confirm(
        'Are you sure? This will delete all feature flag configurations for this environment.'
      )
    )
      return
    try {
  ...
  const handleReorderEnvironment = async (
    envId: string,
    direction: 'up' | 'down'
  ) => {
    ...
    ;[newEnvironments[index], newEnvironments[targetIndex]] = [
      newEnvironments[targetIndex],
  ...
  const handleUpdateBrand = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setBrands(
        brands.map((b) => (b.id === editingBrand.id ? response.data : b))
      )
      setShowBrandDialog(false)
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <Button
          ...
          size="icon"
          onClick={() =>
            navigate(`/projects/${projectId}?orgId=${project.organizationId}`)
          }
        >
            ...
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    value={projectType}
                    onValueChange={(v: 'SINGLE' | 'MULTI') => setProjectType(v)}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Multi-tenant projects allow you to manage feature flags for
                    multiple brands independently.
                  </p>
                </div>
                <div className="space-y-2">
                        ...
                        <>
                          <div className="flex items-center border rounded-md overflow-hidden mr-2">
                            <Button
                              ...
                              disabled={index === 0}
                              onClick={() =>
                                handleReorderEnvironment(env.id, 'up')
                              }
                            >
                            ...
                            <Button
                              ...
                              onClick={() =>
                                handleReorderEnvironment(env.id, 'down')
                              }
                            >
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        <Button
                          ...
                          onClick={() =>
                            navigate(
                              `/projects/${projectId}/brands/${brand.id}/flags`
                            )
                          }
                        >
                          Manage Flags
            ...
            <CardHeader>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <CardDescription>
                    Configure allowed origins for SDK access
                  </CardDescription>
                </div>
              </div>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="bg-muted/50 p-4 rounded-lg">
                ...
                <p className="text-sm text-muted-foreground">
                  Specify which domains are allowed to access your feature flags
                  via the SDK. Leave empty to allow all origins, or add specific
                  domains for enhanced security.
                </p>
                <ul className="text-sm text-muted-foreground mt-2 list-disc list-inside">
                  <li>
                    Use <code>https://example.com</code> for exact match
                  </li>
                  <li>
                ...
                <div className="space-y-2">
                  {allowedOrigins.length === 0 ? (
                    <p className="text-sm text-muted-foreground italic">
                      No origins configured - all origins will be allowed
                    </p>
                  ) : (
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription className="space-y-4">
              <p>
                You are about to change this project from{' '}
                <strong>Multi-Tenant</strong> to <strong>Single-Tenant</strong>.
              </p>
              ...
              <p className="text-sm text-muted-foreground">
                The default flag values will be preserved. This action cannot be
                undone.
              </p>
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={showEnvDialog} onOpenChange={setShowEnvDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {editingEnv ? 'Rename Environment' : 'Add Environment'}
            </DialogTitle>
          </DialogHeader>
          <form
            onSubmit={
              editingEnv ? handleUpdateEnvironment : handleCreateEnvironment
            }
          >
            ...
            <DialogFooter>
              ...
              <Button type="submit">{editingEnv ? 'Save' : 'Add'}</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent>
          ...
          <form onSubmit={editingBrand ? handleUpdateBrand : handleCreateBrand}>
            ...
            <DialogFooter>
              ...
              <Button type="submit">{editingBrand ? 'Save' : 'Add'}</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
