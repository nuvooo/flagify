...
async function runTests() {
  ...
  const singleResult = await testEndpoint(singleFlagUrl, 'Single Flag (with API Key)');
  ...
  await testEndpoint(noKeyUrl, 'Single Flag (NO API Key - should fail with 401)');
  
  // Test 5: Wrong API key
  const wrongKeyUrl = `${BASE_URL}/sdk/flags/${PROJECT}/${ENV}/${FLAG}?apiKey=wrong_key_123`;
  await testEndpoint(wrongKeyUrl, 'Single Flag (WRONG API Key - should fail with 401)');
  ...
  if (!singleResult.success) {
    switch(singleResult.data?.code) {
      ...
      case 'INVALID_API_KEY':
        console.log('🔑 API Key is invalid! Check the key in Togglely Dashboard.');
      ...
      case 'ORIGIN_NOT_ALLOWED':
        console.log('🌐 Origin not allowed! Add your domain to Project Settings > Allowed Origins.');
        break;
    ...
    console.log('');
    console.log('   OR the flag might not exist yet (auto-created as disabled).');
  } else {
