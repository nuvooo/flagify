...
import { TogglelyClient, type TogglelyConfig } from '@togglely/sdk-core'
...
export async function isEnabled(
  ...
  if (!client) {
    console.error(
      '[Togglely] No global instance found. Call initTogglely() first.'
    )
    return defaultValue
  }
...
export function togglelyInit(
  config: Record<
    ...
    | string
    | string[]
    | { selector: string; defaultValue?: boolean; invert?: boolean }
  >
): () => void {
  const client = getGlobalTogglely()
  if (!client) {
