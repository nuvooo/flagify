...
export function useTogglelyClient(): TogglelyClient {
  ...
  if (!client) {
    throw new Error(
      'useTogglelyClient must be used after installing the Togglely plugin'
    )
  }
  return client
}
 ...
 */
export function useNumberToggle(
  key: string,
 ...
 */
export function useToggles(): DeepReadonly<
  Ref<Record<string, { value: any; enabled: boolean }>>
> {
  const client = useTogglelyClient()
...
export const vFeatureToggle = {
  mounted(el: HTMLElement, binding: any) {
    const client =
      (binding.instance?.$togglely as TogglelyClient) ||
      inject<TogglelyClient>(TOGGLELY_CLIENT_KEY)

    if (!client) {
      console.error(
        '[Togglely] Client not found. Make sure to install the Togglely plugin.'
      )
  ...
  updated(el: HTMLElement, binding: any) {
    ...
    client
      .isEnabled(config.toggle, config.defaultValue ?? false)
      .then((isEnabled) => {
        el.style.display = isEnabled ? '' : 'none'
      })
  },
