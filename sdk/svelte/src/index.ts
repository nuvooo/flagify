...
export function initTogglely(config: TogglelyConfig): TogglelyClient {
  if (globalClient) {
    console.warn(
      '[Togglely] Client already initialized, destroying old instance'
    )
 ...
 */
export function jsonToggle<T = any>(
  key: string,
  defaultValue: T = {} as T
): Readable<T> {
  const client = getTogglelyClient()
 ...
 */
export function toggles(): Readable<
  Record<string, { value: any; enabled: boolean }>
> {
  const client = getTogglelyClient()

  return readable<Record<string, { value: any; enabled: boolean }>>(
    {},
    (set) => {
...
export function createToggle(
  ...
  client.isEnabled(key, defaultValue).then((v) => {
    value = v
  })
