...
async function test() {
  ...
  console.log(
    'Project:',
    project
      ? { id: project.id, key: project.key, type: project.type }
      : 'NOT FOUND'
  )
  ...
  console.log(
    'Flag:',
    flag ? { id: flag.id, key: flag.key, flagType: flag.flagType } : 'NOT FOUND'
  )
  ...
  if (flag && env) {
    ...
    console.log(
      '\nFlagEnvironment (with brand muffenrohr):',
      feBrand
        ? {
            id: feBrand.id,
    ...
    console.log(
      'FlagEnvironment (default, no brand):',
      feDefault
        ? {
            id: feDefault.id,
